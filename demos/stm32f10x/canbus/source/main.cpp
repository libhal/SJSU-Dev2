...
int main(void)
{
  ...
  sjsu::LogInfo(
      "example will transmit and receive messages on CAN 1.");
  ...
  while (true)
  {
    if (can1.HasData())
      {
        ...
        sjsu::LogInfo("Recieved Message -  %d | %#04x %#04x %#04x %#04x %#04x %#04x %#04x %#04x",
                      received_message.id,
    ...
    if(count == 30)
    {
      ...
      test_message.payload[1] = static_cast<unsigned char>(0x01);
      test_message.payload[2] = static_cast<unsigned char>(0x01);
      test_message.payload[3] = static_cast<unsigned char>(0x01);
      test_message.payload[4] = static_cast<unsigned char>(0x01);
      test_message.payload[5] = static_cast<unsigned char>(0x01);
      test_message.payload[6] = static_cast<unsigned char>(0x01);
      test_message.payload[7] = static_cast<unsigned char>(0x01);
