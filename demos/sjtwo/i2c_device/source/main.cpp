...
int main()
{
  sjsu::LogInfo("I2C MemoryMapped Device Starting...");
  ...
  while (true)
  {
    ...
    sjsu::LogInfo("x = 0x%02X%02X", x_msb, x_lsb);

    uint8_t reg1;
    reg1 = gesture.memory.adc_integration_time;
    sjsu::LogInfo("cur reg1 = 0x%02X", reg1);

    gesture.memory.adc_integration_time = 5;
    reg1                                = gesture.memory.adc_integration_time;
    sjsu::LogInfo("new reg1 = 0x%02X", reg1);

    gesture.memory.adc_integration_time |= (1 << 7);
    reg1 = gesture.memory.adc_integration_time;
    sjsu::LogInfo("ord reg1 = 0x%02X", reg1);

    gesture.memory.adc_integration_time = 255;
    reg1                                = gesture.memory.adc_integration_time;
    sjsu::LogInfo("add reg1 = 0x%02X", reg1);
    sjsu::LogInfo("========================\n");
