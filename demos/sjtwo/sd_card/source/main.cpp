...
int main()
{
  ...
  sjsu::LogInfo("Writing Hello World to block 0 (0x00000000)");
  card.WriteBlock(0x00000000, kHelloWorld);

  sjsu::Delay(1s);

  sjsu::LogInfo("Reading block 0 (0x00000000)");
  card.ReadBlock(0x00000000, buffer);
  sjsu::debug::Hexdump(buffer, sizeof(buffer));

  sjsu::Delay(1s);

  sjsu::LogInfo("Deleting blocks again");
  card.DeleteBlock(0x00000000, 0x0000A000);

  sjsu::Delay(1s);

  sjsu::LogInfo("Reading block 0 after delete (0x00000000)");
  card.ReadBlock(0x00000000, buffer);
  sjsu::debug::Hexdump(buffer, sizeof(buffer));

  sjsu::LogInfo("Writing to block 1 after delete (0x00000200)");
  card.WriteBlock(0x00000200, kLongText);

  sjsu::Delay(1s);

  sjsu::LogInfo("Reading block 1 after write (0x00000200)");
