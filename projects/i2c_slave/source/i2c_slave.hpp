...
#include "L0_Platform/LPC40xx.h"
#include "L0_Platform/system_controller.hpp"
#include "L1_Pe/pin.hpp"
...
class I2c final : public I2cInterface, protected Lpc40xxSystemController
{
 public:
  ...
  enum class MasterState : uint32_t
  {
    ...
    kDoNothing                         = 0xF8,
    // Slave States
    kOwnSlaveWriteAddress = 0x60,
    kOwnSlaveReadAddress  = 0xA8,
  };
  ...
  static void I2cHandler(Port port)
  {
    ...
    switch (state)
    {
      ...
      // Slave States
      case MasterState::kOwnSlaveWriteAddress:  // 0x60
      {
        i2c[kPort]->DAT = 0;
        clear_mask      = Control::kAssertAcknowledge;
        printf("SLA+W\n");
      ...
      case MasterState::kOwnSlaveReadAddress:  // 0xA8
      {
        i2c[kPort]->DAT = 0;
        clear_mask      = Control::kAssertAcknowledge;
        printf("SLA+R\n");
      ...
      case MasterState::kOwnAddressReceived:  // 0xA80
      {
        clear_mask = Control::kStart | Control::kAssertAcknowledge;
        printf("SLA\n");
        break;
      }
      default:
      {
        ...
        SJ2_ASSERT_FATAL(false, "Invalid I2C State Reached 0x%02X!!", state);
        break;
      }
    }
  ...
  void InitializeSlave()
  {
    ...
    // static_cast<float>(GetPeripheralFrequency()); float scll         =
    // ((peripheral_frequency / 75'000.0f) / 2.0f) * 0.7f; i2c[port_]->SCLL   =
    // static_cast<uint32_t>(scll); float sclh         = ((peripheral_frequency
    // / 75'000.0f) / 2.0f) * 1.3f; i2c[port_]->SCLH   =
    // static_cast<uint32_t>(sclh);

    i2c[port_]->CONCLR = Control::kStart | Control::kStop | Control::kInterrupt;

    i2c[port_]->CONSET =
        Control::kInterfaceEnable | Control::kAssertAcknowledge;
    i2c[port_]->ADR0 = 0b0100'1110;
    i2c[port_]->ADR1 = 0b0100'1110;
    i2c[port_]->ADR2 = 0b0100'1110;
    i2c[port_]->ADR3 = 0b0100'1110;
    // i2c[port_]->MASK0 = 0b1111'1110;
    RegisterIsr(kIrq[port_], kHandlers[port_], true);
    initialized_ = true;
  }
