...
//    implementation is a standard and all SD cards must follow it. The types of
//    communication protocols are also standard (SPI in this case), so as long
//    as the SD card driver can accept an SPI peripheral, then it can work.
class ExampleHal
{
 public:
...
//
//    The folder hierachy should be something like this:
//
//    category/
//    |
...
//         |
//         +-- <hal_name>_gpio.hpp (implementation for gpio)
//         +-- <hal_name>_i2c.hpp  (implementation for i2c)
//         +-- <hal_name>_spi.hpp  (implementation for spi)
//         +-- <hal_name>_etc.hpp  (implementation for whatever else)
