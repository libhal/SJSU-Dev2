...
// Compiler versions:
//
// MSVC++  6.0  _MSC_VER == 1200  nsrs_COMPILER_MSVC_VERSION ==  60  (Visual
// Studio 6.0) MSVC++  7.0  _MSC_VER == 1300  nsrs_COMPILER_MSVC_VERSION ==  70
// (Visual Studio .NET 2002) MSVC++  7.1  _MSC_VER == 1310
// nsrs_COMPILER_MSVC_VERSION ==  71  (Visual Studio .NET 2003) MSVC++  8.0
// _MSC_VER == 1400  nsrs_COMPILER_MSVC_VERSION ==  80  (Visual Studio 2005)
// MSVC++  9.0  _MSC_VER == 1500  nsrs_COMPILER_MSVC_VERSION ==  90  (Visual
// Studio 2008) MSVC++ 10.0  _MSC_VER == 1600  nsrs_COMPILER_MSVC_VERSION == 100
// (Visual Studio 2010) MSVC++ 11.0  _MSC_VER == 1700 nsrs_COMPILER_MSVC_VERSION
// == 110  (Visual Studio 2012) MSVC++ 12.0  _MSC_VER == 1800
// nsrs_COMPILER_MSVC_VERSION == 120  (Visual Studio 2013) MSVC++ 14.0  _MSC_VER
// == 1900  nsrs_COMPILER_MSVC_VERSION == 140  (Visual Studio 2015)
// ............ _MSVC_LANG: 201402 for -std:c++14, default
// MSVC++ 14.1  _MSC_VER >= 1910  nsrs_COMPILER_MSVC_VERSION == 141  (Visual
// Studio 2017)
// ............ _MSVC_LANG: 201402 for -std:c++14, default
// ............ _MSVC_LANG: 201703 for -std:c++17
// MSVC++ 14.2  _MSC_VER >= 1920  nsrs_COMPILER_MSVC_VERSION == 142  (Visual
// Studio 2019)
...
#endif
{
 ...
 public:
  ...
  nsrs_REQUIRES_0(std::is_copy_assignable<T>::value) void push_back(
      value_type const &
          value) noexcept(std::is_nothrow_copy_assignable<T>::value)
#else
...
#if nsrs_CPP11_OR_GREATER
  nsrs_REQUIRES_0(std::is_move_assignable<T>::value) void push_front(
      T && value) noexcept((std::is_nothrow_move_assignable<T>::value))
  {
    if (full())
      decrement_front_and_back_();
    else
 ...
 private:
  ...
  size_type normalize_(size_type const idx,
                       std11::true_type) const nsrs_noexcept
  {
    return idx & (m_capacity - 1);
  }

  size_type normalize_(size_type const idx,
                       std11::false_type) const nsrs_noexcept
  {
...
class ring_iterator
{
 public:
  ...
  typedef typename std11::conditional<is_const, const value_type, value_type>::
      type * pointer;
  typedef typename std11::conditional<is_const, const value_type, value_type>::
      type & reference;
...
inline ring_iterator<RS, C> operator+(ring_iterator<RS, C> it,
                                      int i) nsrs_noexcept
{
  it += i;
  return it;
