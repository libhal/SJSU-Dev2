...
TEST_CASE("Testing EEPROM", "[Eeprom]")
{
    ...
    constexpr uint32_t kDummySystemClockFrequency = 48'000'000;
    ...
    SECTION("Writing")
    {
        ...
        constexpr uint32_t kExpectedValue = (wdata[3] << 24) +
                                            (wdata[2] << 16) +
                                            (wdata[1] << 8) +
                                            (wdata[0]);
    ...
    SECTION("Reading")
    {
        ...
        uint32_t kExpectedValue = (rdata[3] << 24) +
                                  (rdata[2] << 16) +
                                  (rdata[1] << 8) +
                                  (rdata[0]);
        CHECK(local_eeprom.ADDR == kAddress);
        CHECK(local_eeprom.CMD == Eeprom::kRead32Bits);
        CHECK(kExpectedValue == kReadVal);
