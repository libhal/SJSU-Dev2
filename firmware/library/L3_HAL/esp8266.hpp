...
class Esp8266WifiInterface
{
 public:
  ...
  virtual bool ConnectToAccessPoint(
    const char ssid[], const char password[])                = 0;
  ...
  virtual bool SendGetRequest(
    const char url[], char buffer[] = default_buffer.data(),
    std::size_t buffer_size = default_buffer.size())         = 0;
  virtual bool SendPostRequest(
    const char url[], const char data[],
    char buffer[] = default_buffer.data(),
    std::size_t buffer_size = default_buffer.size())         = 0;
};
