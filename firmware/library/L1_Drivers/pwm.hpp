...
class Pwm : public PwmInterface
{
 public:
    ...
    static constexpr uint8_t kGpioFunction = 0b000;
    static volatile uint32_t * match_register_table[7];
    ...
    {
        static_assert(pwm_channel <= 6,
                      "Available PWM channels only go up to pin 6.");
    ...
    ~Pwm()
    {
        pwm_pin_.SetPinFunction(kGpioFunction);
    }
    ...
    void Initialize(uint32_t frequency_hz = 1'000) override
    {
        ...
        // DIRECT MANIUPULATION OF SYSTEM CLOCK REGISTERS
        // WILL REPLACE WITH FUNCTIONS LATER
        // Enables PWM1 power/clock control bit
        sc->PCONP |= SysClk::kActivatePwm1;
        // TODO(delwin-lei): Remove when sysclock complete.
    ...
    {
        return (static_cast<float>(*match_register_table[channel_]) /
