...
TEST_CASE("Testing I2C", "[i2c]")
{
    ...
    SECTION("Initialize")
    {
        ...
        Verify(
            Method(mock_sda_pin, SetPinFunction).Using(I2c::kI2cPort2Function));
        Verify(Method(mock_sda_pin, SetAsOpenDrain));
        Verify(
            Method(mock_sda_pin, SetMode).Using(PinInterface::Mode::kInactive));

        Verify(
            Method(mock_scl_pin, SetPinFunction).Using(I2c::kI2cPort2Function));
        Verify(Method(mock_scl_pin, SetAsOpenDrain));
        Verify(
            Method(mock_scl_pin, SetMode).Using(PinInterface::Mode::kInactive));
    }
    SECTION("Read Setup")
    {
    ...
    SECTION("I2C State Machine: kSlaveAddressWriteSentRecievedAck")
    {
        ...
        {
            setup_state_machine(
                I2c::MasterState::kSlaveAddressWriteSentRecievedAck);
    ...
    {
        setup_state_machine(
            I2c::MasterState::kSlaveAddressWriteSentRecievedNack);
    ...
    SECTION("I2C State Machine: kSlaveAddressReadSentRecievedAck")
    {
        ...
        {
            setup_state_machine(
                I2c::MasterState::kSlaveAddressReadSentRecievedAck);
    ...
    {
        setup_state_machine(
            I2c::MasterState::kSlaveAddressReadSentRecievedNack);
