...
class OnBoardLedInterface {
 public:
  ...
  kOff = false
           ...
           virtual void Set(uint8_t led_number,
              OnBoardLedInterface::LightState state)                      = 0;
...
class OnBoardLed: public OnBoardLedInterface
{
 public:
  ...
  // then turns all of the leds by setting the output high.
  ...
  {
    if (led_number > 3) {printf("Input Led number can't be greater than 3.");}
  ...
  void Set(uint8_t led_number,
     OnBoardLedInterface::LightState state = kOn) override
  {
    if (led_number > 3) {printf("Input Led number can't be greater than 3.");}
    (state) ? On(led_number) : Off(led_number);
  }
  ...
  void Toggle(uint8_t led_number) override
  {
    ...
    (swap) ? On(led_number) : Off(led_number);
  }
  ...
  void SetAll(uint8_t value) override
  {
    for (uint8_t i = 0; i < 4; i++)
    {
      ((value >> i)&1) ? led[i].SetLow() : led[i].SetHigh();
    }
  }
