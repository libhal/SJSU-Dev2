...
class SystemClock : public SystemClockInterface
{
 ...
 public:
  ...
  {
      LPC_SC->CCLKSEL = (LPC_SC->CCLKSEL & ~(kBaseClockSelect)) |
          static_cast<uint32_t>(source);
  }
  ...
  uint32_t CalculatePll(PllInput input_frequency, uint16_t desired_speed_in_mhz)
  {
      ...
      {
          multiplier_value = static_cast<uint32_t>((desired_speed_in_mhz /
               static_cast<uint16_t>(input_frequency)) + 1);
      }
      else
      {
          multiplier_value = static_cast<uint32_t>(desired_speed_in_mhz /
               static_cast<uint16_t>(input_frequency));
      }
      uint16_t divider_value = 1;
      while (calculating)
      {
          uint16_t current_controlled_oscillator_frequency;
          current_controlled_oscillator_frequency =
               static_cast<uint16_t>((static_cast<uint16_t>(input_frequency) *
               multiplier_value * 2) * divider_value);
  ...
  uint32_t SetMainPll(PllInput input_frequency, uint16_t desired_speed_in_mhz)
  {
      ...
      uint32_t multiplier_value = CalculatePll(input_frequency,
           desired_speed_in_mhz);
      uint32_t actual_speed = static_cast<uint32_t>(input_frequency) *
           multiplier_value;
      ...
      LPC_SC->PLL0CFG = (LPC_SC->PLL0CFG & ~kClearPllMultiplier) |
           (multiplier_value - 1);
      LPC_SC->PLL0CFG = (LPC_SC->PLL0CFG & ~kClearPllDivider) |
           (divider_value << 5);
      ...
      uint16_t desired_speed_in_mhz)
  {
      ...
      SelectSpifiClockSource(SpifiSource::kBaseClock);
      // must subtract 1 from multiplier value as specified in datasheet
      LPC_SC->PLL1CFG = (LPC_SC->PLL1CFG & ~kClearPllMultiplier) |
           (multiplier_value - 1);
      LPC_SC->PLL1CFG = (LPC_SC->PLL1CFG & ~kClearPllDivider) |
           (divider_value << 5);
  ...
  void SetPeripheralClockDivider(uint8_t peripheral_divider)
  {
      ...
      LPC_SC->PCLKSEL = (LPC_SC->PCLKSEL & ~kClearPeripheralDivider) |
          peripheral_divider;
  }
  ...
  {
     LPC_SC->PCONP = LPC_SC->PCONP |
          (1 << static_cast<uint8_t>(peripheral_select));
  }
