...
class Adc : public AdcInterface
{
 public:
    ...
    const byte kAdcPorts[kMaxNumOfPins] =
    {
        kChannel0Port, kChannel1Port, kChannel2Port, kChannel3Port,
        kChannel4Port, kChannel5Port, kChannel6Port, kChannel7Port
    };
    const uint8_t kAdcPins[kMaxNumOfPins] =
    {
        kChannel0Pin, kChannel1Pin, kChannel2Pin, kChannel3Pin,
        kChannel4Pin, kChannel5Pin, kChannel6Pin, kChannel7Pin
    };
    ...
    void Conversion() override
    {
        ...
        while (!(adc_base->GDR & (1 << kDoneBit))) { continue; }
    }
    uint16_t ReadResult() override
    {
        ...
        /*result = uint16_t(adc_base->GDR >> 4);
        result &= kResultMask;

        return result;*/

        return (uint16_t(adc_base->GDR >> 4) & kResultMask);
    }
